...
kmeans = KMeansSparseCluster(x=normalized_df, K=4)
summary(kmeans)
normalized_df$cluster = kmeans[[2]]$Cs
ggplot(data = normalized_df, aes(x = ACO2, y = PLIN4, color = factor(cluster))) +
  geom_point() +
...
plot_ly(data = normalized_df, x = ~ACO2, y = ~PLIN4, z = ~RPS10, color = ~cluster, colors = "Set1") %>%
  add_markers() %>%
  layout(scene = list(
